...
def clear_symbol_cache():
    ...
    get_symbol_info_cached.cache_clear()
...
def validate_symbol(symbol: str) -> Tuple[bool, Optional[str], Optional[Dict[str, Any]]]:
    """
    ...
    if symbol_info is None:
        ...
        return False, error_msg, {"suggestion": suggestion, "corrected_params": corrected}
...
def convert_order_type(order_type_str: str) -> int:
    ...
    if ot not in ORDER_TYPE_MAP:
        raise ValueError(
            f"Invalid order_type: '{order_type_str}'. Valid: buy, sell"
        )
...
def validate_and_adjust_volume(
    ...
    if abs(volume - original) > 1e-9:
        warning = f"Volume adjusted from {original} to {volume} (min={vol_min}, max={vol_max}, step={vol_step})"
...
def validate_indicator_data_requirements(
    ...
    if bar_count < required:
        return False, f"Need at least {required} bars for {indicator_function}, got {bar_count}. Try: count={required}"
...
def validate_ta_function(function_path: str) -> Tuple[bool, Optional[str]]:
    ...
    if len(parts) != 3:
        return False, f"Function path must be 'ta.<module>.<function>', got: '{function_path}'"
    ...
    try:
        ...
        if not hasattr(ta, module_name):
            return False, f"TA-Lib module '{module_name}' not found. Available: momentum, trend, volatility, volume, others"
