...
try:
    ...
    print(f"✓ safe_mt5_call function available")
except Exception as e:
...
try:
    from mt5_mcp.models import (
        MT5QueryRequest, MT5Operation, 
        MT5AnalysisRequest, IndicatorSpec,
        ChartConfig, ForecastConfig
    )
    
    # Create a sample request
    req = MT5QueryRequest(
        operation=MT5Operation.SYMBOL_INFO,
        symbol="BTCUSD"
    )
...
try:
    from mt5_mcp.gradio_server import (
        set_rate_limit, check_rate_limit,
        mt5_query_tool, mt5_analyze_tool, mt5_execute_tool
    )
    print("✓ Gradio server module imported")
    print("✓ Rate limiting functions available")
    print("✓ All MCP tools available: query, analyze, execute")
...
try:
    ...
    print("✓ Multi-transport functions available")
except Exception as e:
...
try:
    ...
    with open(os.path.join(os.path.dirname(__file__), '..', 'pyproject.toml'), 'rb') as f:
        ...
        print("✓ Version is 0.5.0 (Gradio MCP integration)")
except ImportError:
